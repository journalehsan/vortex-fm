...
flamegraph *args:
    cargo flamegraph --release --bin vortex-fm -- --no-daemon {{args}}
    xdg-open flamegraph.svg

heaptrack *args:
    #!/usr/bin/env bash
    set -ex
    rm -fv heaptrack.vortex-fm.*
    cargo heaptrack --profile release-with-debug --bin vortex-fm -- --no-daemon {{args}}
    zstd -dc < heaptrack.vortex-fm.*.raw.zst | /usr/lib/heaptrack/libexec/heaptrack_interpret | zstd -c > heaptrack.vortex-fm.zst
    heaptrack_gui heaptrack.vortex-fm.zst
